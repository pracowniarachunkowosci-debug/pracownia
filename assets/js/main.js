...
['utm_source','utm_medium','utm_campaign','utm_term','gclid','fbclid'].forEach(param=>{
  const val = urlParams.get(param);
  if(val) {
    document.querySelector(`input[name="${param}"]`).value = val;
  }
});
form.addEventListener('submit', ()=>{
  gtag('event','generate_lead',{ /* opcjonalnie parametry lead_source etc. */ });
});

  });
// Istniejący kod zachowany

// === ZGODA COOKIES + ANALITYKA ===
function loadGA4() {
  const s = document.createElement('script');
  s.src = 'https://www.googletagmanager.com/gtag/js?id=G-SLZ3YX2T0K';
  s.async = true;
...
// Obsługa kliknięć zgody
const acceptAll = document.querySelector('#accept-all');
const acceptEssential = document.querySelector('#accept-essential');
...
// Automatyczne załadowanie jeśli wcześniej zaakceptowano
